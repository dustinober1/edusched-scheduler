...
def export_to_json(result: "Result", output_path: Path) -> None:
    ...
    for assignment in result.assignments:
        assignment_data = {
            "request": {
                ...
                "course_code": getattr(
                    assignment.request, "course_code", str(assignment.request.id)
                ),
    ...
    if result.assignments:
        ...
        export_data["statistics"] = {
            ...
            "utilization_rate": len(result.assignments) / len(result.problem.resources)
            if result.problem.resources
            else 0,
        }
...
def export_to_ical(result: "Result", output_path: Path) -> None:
    ...
    try:
        ...
        raise ImportError(
            "icalendar package is required for iCal export. Install with: pip install icalendar"
        )
    ...
    cal.add(
        "description", f"Schedule generated by EduSched on {datetime.now().strftime('%Y-%m-%d')}"
    )
    ...
    for assignment in result.assignments:
        ...
        event.add(
            "uid",
            f"{assignment.request.id}@{assignment.resource.id}@{assignment.start_time.isoformat()}",
        )
...
def export_to_excel(result: "Result", output_path: Path) -> None:
    ...
    try:
        ...
        raise ImportError(
            "pandas package is required for Excel export. Install with: pip install pandas openpyxl"
        )
        ...
        row = {
            ...
            "Utilization %": round(
                (
                    ...
                    / max(getattr(assignment.resource, "capacity", 1), 1)
                )
                * 100,
                1,
            ),
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        summary_data = {
            ...
            "Value": [
                ...
                sum(getattr(a.request, "enrollment", 0) for a in result.assignments)
                / max(len(result.assignments), 1),
                sum(
                    (
                        getattr(a.request, "enrollment", 0)
                        / max(getattr(a.resource, "capacity", 1), 1)
                    )
                    * 100
        ...
        if result.assignments:
            ...
            for assignment in result.assignments:
                ...
                room_usage[room_id]["Total Enrollment"] += getattr(
                    assignment.request, "enrollment", 0
                )
